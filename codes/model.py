...
### MODIFIED VERSION BASED ON TiSASRec model code
### Experiments on BASELINE SASRec are also done with this version 
...
class SASRec(torch.nn.Module): # similar to torch.nn.MultiheadAttention
    ...
    def seq2feats(self, log_seqs):
        seqs = self.item_emb(torch.LongTensor(log_seqs).to(self.dev))     ## (batch_size x max_len x emb_dim)
    ...
    def predict(self, user_ids, log_seqs, item_indices): # for inference
        ...
        log_feats = self.seq2feats(log_seqs) # user_ids hasn't been used yet
    ...
    ###
class SASRec_(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        super(SASRec_, self).__init__()
    ...
    def log2feats(self, log_seqs):
        ...
        seqs = self.emb_dropout(seqs)              ## dropout( item_seq emb + abs_pos_emb ) 을 Q,K,V 로 사용
